...
class CommandExecutor:
    ...
    def _execute_single_command(self, cmd_name: str, cmd: str, stdout_path: str, stderr_path: str, 
        ...
        try:
            ...
            self.program_manager.add_command_to_domain(program_uuid, target_uuid, command_details)
    ...
    def kill_domain_processes(self, program_uuid, target_uuid: str) -> List[int]:
        ...
        try:
            ...
            logger.debug(f"Killed {len(killed_pids)} processes for domain UUID: {target_uuid}")
            self.program_manager.update_domain_status_by_id(program_uuid, target_uuid, "completed")
    ...
    def kill_program_processes(self, program_uuid: str) -> List[int]:
        ...
        try:
            processes = self.process_manager.get_all_processes()
            program_processes = {pid: info for pid, info in processes.items() 
            ...
            logger.debug(f"Killed {len(killed_pids)} processes for program UUID: {program_uuid}")
            return killed_pids
            
        except Exception as e:
            logger.error(f"Error killing processes for program {program_uuid}: {str(e)}")
            return killed_pids

    def execute_commands(self, program_name: str, domain: str, commands: List[Tuple], program_uuid: str, target_uuid: str, scan_dir: str, execution_style: Literal['sequential', 'parallel'] = 'sequential') -> None:
        """Execute commands either sequentially or in parallel."""
        try:
            if not self.program_manager.create_program(program_name, program_uuid):
                self.program_manager.update_program_status_by_id(program_uuid, new_status="running")
            ...
            if execution_style == 'parallel':
                ...
                for cmd_name, cmd, stdout_path, stderr_path in commands:
                    thread = threading.Thread(
                        ...
                        args=(cmd_name, cmd, stdout_path, stderr_path, program_uuid, target_uuid)
                    )
...
def run_commands(program_name: str, domain: str, commands: List[Tuple], program_uuid: str, target_uuid: str, scan_dir: str, 
                execution_style: Literal['sequential', 'parallel'] = 'sequential',) -> None:
    """Main function to run commands."""
    executor = CommandExecutor()
    executor.execute_commands(program_name, domain, commands, program_uuid, target_uuid, scan_dir, execution_style)