...
class FieldTracker:
    ...
    def track_changes(self, previous: Dict[str, Any], current: Dict[str, Any]) -> List[ChangeMetadata]:
        ...
        # for field, compare_func in self.fields_to_track.items():
        #     old_field = f'old_{field}'
        #     new_field = f'new_{field}'
            
        #     prev_new_value = previous.get(new_field)
        #     curr_new_value = current.get(new_field)
            
        #     if compare_func(prev_new_value, curr_new_value):
        #         changes.append(ChangeMetadata(
        #             field_name=field,
        #             old_value=prev_new_value,
        #             new_value=curr_new_value
        #         ))
...
class EndpointChangeDetector:
    ...
    def _rotate_data(self, previous_data: Dict[str, Any], current_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _handle_file_rotation(self, previous_data: Dict[str, Any], current_data: Dict[str, Any]) -> None:
                ...
                if os.path.exists(old_path):
                    if previous_data.get('old_body_file_path'):
                        try:
                            ...
                            logger.debug(f"Removed old file: {previous_data['old_body_file_path']}")
                        except OSError as e:
                            logger.warning(f"Could not remove old file: {e}")
                            
                logger.debug(f"File rotation complete. New file: {new_path}, Old file: {old_path}")
            except Exception as e:
                logger.exception(f"Error during file rotation: {e}")

    def detect_and_update_changes(self, previous_data: Dict[str, Any], current_data: Dict[str, Any]) -> Tuple[bool, List[str]]:
        """
        ...
        try:
            ...
            if changes:
                ...
                if previous_data['old_status_code'] is not None or previous_data['new_status_code'] is not None:
                    updated_data['change_detected_at'] = datetime.now(pytz.timezone("Asia/Kolkata"))
                ...
                logger.debug(f"No changes detected for {updated_data['url']}, timestamp updated")
                return False, []
                
        except Exception as e:
            logger.exception(f"Error in change detection for {current_data.get('url', 'unknown URL')}: {e}")
            raise

    def _update_database(self, endpoint_id: str, update_data: Dict[str, Any], changes: List[ChangeMetadata]) -> None:
