...
def arrangeUrls(urls_path, urlsArranged200_path, urlsArrangedAll_path):
    ...
    directory_path = os.path.dirname(urlsArranged200_path)
    def check_url(url):
        try:
...
def arrangeUrls_small(urls_path, urlsArranged200_small_file_path):
    ...
    def process_extension(ext, urls, urlsArranged200_small_file_path):
        arranged_urls = [url.strip() for url in urls if re.search(rf"\.{ext}(\?.*)?$", url, re.IGNORECASE)]
        if arranged_urls:
            with open(urlsArranged200_small_file_path, 'a') as file200:
                file200.write(f"================(.{ext})\n\n")
                for url in arranged_urls:
                    file200.write(f"{url}\n")
                file200.write("\n\n")
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        futures = [executor.submit(process_extension, ext, urls, urlsArranged200_small_file_path) for ext in extensions_small]
        for future in as_completed(futures):
            try:
                future.result()
            except Exception as e:
...
def run_small(urlsArranged200_file_path):
    global directory_path

    urlsArranged200_urlsArranged200_small_file_path = os.path.join(directory_path, urlsArranged200_small)
    arrangeUrls_small(urlsArranged200_file_path, urlsArranged200_urlsArranged200_small_file_path)
...
if __name__ == "__main__":
    ...
    run_small(args.urlsArranged200_path)