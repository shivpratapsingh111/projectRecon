...
def func_urls_ps(
    ...
    domain_list,
    execution_style,
    include_api, # DO NOT REMOVE THIS
    tool_selection,
    selected_tools,
    program_uuid,
    target_uuid_list,
    ...
    for domain, target_uuid in zip(domain_list, target_uuid_list):
        ...
        with open(
            f"{ROOT_DATA_DIR}/{program_name}/{CENTRAL_LOG_FILE}", "a"
        ) as writeLog:
...
def organise_urls(program_name, domain_list, program_uuid, target_uuid_list):
    ...
    for domain, target_uuid in zip(domain_list, target_uuid_list):
        ...
        commands = [
            (
                "Extracting JS Urls",
                f"""cd {result_dir} ; cat {urls_file} | grep -i .js$ | cut -d'?' -f1 | cut -d'#' -f1 | sort -u >> {js_urls}""",
...
def start_urls_scan(
    program_name, domain_list, execution_style, config, program_uuid, target_uuid_list
):
