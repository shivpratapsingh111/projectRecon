...
class MobileTarget(BaseModel):
    program_uuid: str = Field(..., example="0d7228af-154e-4423-84d6-4761efc6e59b")
...
async def api_add_mobile_target(data: MobileTarget):
    ...
    if not data.target_package and not data.program_uuid:
        return "Target Package adn Program ID is needed"

    mobile_target_data = {
        'program_uuid': str(data.program_uuid),  # Convert UUID to string
