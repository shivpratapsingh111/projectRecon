...
class Command(BaseModel):
    command_name: Optional[str] = Field(default=None, example="subfinder", description="Name of the command/tool being run")
    pid: Optional[int] = Field(default=None, example=1111, description="Process ID of the running command")
    command: Optional[str] = Field(default=None, example="cat subdomains.txt | subfinder", description="The actual command that was executed")
    status: Optional[Literal["pending", "running", "completed", "error", "killed"]] = Field(
        default=None,
        example="completed",
        description="Execution status of the command. Allowed values: 'pending', 'running', 'completed', 'error', 'killed'"
    )
    start_time: Optional[str] = Field(default=None, example="02:04:54, Thursday, 24-04-2025", description="When the command started executing")
    stdout_log: Optional[str] = Field(default=None, example="/var/log/stdout_log.txt", description="Path or content of the standard output log")
    stderr_log: Optional[str] = Field(default=None, example="/var/log/stderr_log.txt", description="Path or content of the standard error log")
    return_code: Optional[int] = Field(default=None, example=0, description="Exit code of the command. Typically 0 = success")
    completion_time: Optional[str] = Field(default=None, example="02:10:54, Thursday, 24-04-2025", description="When the command completed or was terminated")
...
class Program(BaseModel):
    program_name: Optional[str] = Field(
        default=None,
        example="Google",
        description="Name of the program or asset group"
    )
    status: Optional[Literal["pending", "running", "completed", "error", "killed"]] = Field(
        default=None,
        example="completed",
...
class ExistingProgramNamesResponse(BaseModel):
    scan_name: Optional[List[str]] = Field(
        default=None,
        example=["CUCHD", "CULKO", "Cyberboy-Scan", "cuchd", "cyberboy", "newcyberboy", "thecyberboy"]
    )
